...
struct AwsInstance {
  ...
  bool initialize(
    ...
    if (isFinalized_.load()) {
      BOLT_FAIL("Attempt to initialize S3 after it has been finalized.");
    }
...
void finalizeS3() {
  BOLT_CHECK((fileSystemCount == 0), "Cannot finalize S3 while in use");
...
class S3FileSystem::Impl {
 public:
  Impl(const S3Config& s3Config) {
    BOLT_CHECK(getAwsInstance()->isInitialized(), "S3 is not initialized");
    ...
    if (s3Config.connectTimeout().has_value()) {
      clientConfig.connectTimeoutMs =
          std::chrono::duration_cast<std::chrono::milliseconds>(
              bytedance::bolt::config::toDuration(
  ...
  std::optional<std::shared_ptr<Aws::Client::RetryStrategy>> getRetryStrategy(
    ...
    if (retryMode.has_value()) {
      if (retryMode.value() == "standard") {
        ...
        BOLT_USER_FAIL("Invalid retry mode for S3: {}", retryMode.value());
      }
    }
    return std::nullopt;
  }
